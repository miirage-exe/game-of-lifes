...
module.exports = {
    use: function(pack){
        if(!io && pack.server){
            ...
            io? console.error('Obj.server must be a server instance') : console.warn('Didn\'t linked io to server because io is already linked')
        }
        if(!sessionMiddleware && pack.session) {
            sessionMiddleware = pack.session
        } else {
            io? console.error('Obj.session must be a session instance') : console.warn('Didn\'t keep session beceause there is already one')
        }
    },
    Namespace: class {
